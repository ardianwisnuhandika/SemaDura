...
<section class="flex items-center justify-center min-h-screen relative overflow-hidden" style="background: radial-gradient(ellipse at 80% 10%, #b2f7ef 0%, transparent 60%), radial-gradient(ellipse at 10% 90%, #e0c3fc 0%, transparent 70%), linear-gradient(135deg, #e8f5e9 0%, #f1f8e9 50%, #e3f2fd 100%);">
    ...
    <div class="bg-white p-8 rounded-xl shadow-lg w-full max-w-md relative z-10">
        <h2 class="text-2xl font-bold mb-6 text-center text-[#4CAF50]">Registrasi Admin</h2>
        <form method="POST" action="{{ route('admin.register.submit') }}">
            ...
            <button type="submit" class="bg-[#4CAF50] text-white px-6 py-3 rounded-lg w-full font-bold text-lg shadow hover:bg-green-700 transition">Daftar Admin</button>
        </form>
        <div class="mt-4 text-center">
            <a href="{{ route('admin.login') }}" class="text-[#4CAF50] hover:underline">Sudah punya akun? Login di sini</a>
        </div>
    </div>
</section>
@endsection 