...
<section class="flex items-center justify-center min-h-screen relative overflow-hidden" style="background: radial-gradient(ellipse at 80% 10%, #b2f7ef 0%, transparent 60%), radial-gradient(ellipse at 10% 90%, #e0c3fc 0%, transparent 70%), linear-gradient(135deg, #e8f5e9 0%, #f1f8e9 50%, #e3f2fd 100%);">
    ...
    <div class="bg-white p-8 rounded-xl shadow-lg w-full max-w-md relative z-10">
        <h2 class="text-2xl font-bold mb-6 text-center text-[#4CAF50]">Login Admin</h2>
        @if($errors->any())
        ...
        <form method="POST" action="{{ route('admin.login.submit') }}">
            ...
            <div class="mb-4">
                ...
                <input type="email" name="email" class="w-full px-4 py-2 rounded border focus:ring-2 focus:ring-[#4CAF50] focus:border-[#4CAF50] transition" required autofocus value="{{ old('email') }}">
            </div>
            <div class="mb-6">
            ...
            <button type="submit" class="bg-[#4CAF50] text-white px-6 py-3 rounded-lg w-full font-bold text-lg shadow hover:bg-green-700 transition">Login</button>
        </form>
        <div class="mt-4 text-center">
            <a href="{{ route('admin.register') }}" class="text-[#4CAF50] hover:underline">Belum punya akun? Daftar di sini</a>
        </div>
    </div>
</section>
@endsection 